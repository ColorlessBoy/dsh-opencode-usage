 ...
 * route resolves that provider's `baseURL` from the `llm-pi-ai` user-settings
 * section and acts only when it is an OpenCode Zen "Go" endpoint, where the
...
/**
 * Resolve the provider's profile from the `llm-pi-ai` settings section.
...
function providerProfile(ctx, provider) {
  const config = ctx.get('settings')?.get('llm-pi-ai')
