 ...
 * remaining share of each OpenCode Go quota window. The chip is its own dock
 * item seated after the built-in context pill, so it reads as the last pill on
 * that line whether or not the stats row has mounted yet.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Battery glyph for the quota chip. Drawn locally instead of borrowing a
     * primitives icon so a ui-primitives rename cannot crash the dock entry;
     * the 16px line art carries one charge segment beside the full body.
    ...
    function BatteryIcon() {
      ...
      h('rect', { x: 1.5, y: 4.5, width: 11, height: 7, rx: 2 }),
      h('rect', { x: 13.25, y: 6.25, width: 1.5, height: 3.5, rx: 0.75 }),
      h('path', { d: 'M4 8h3' }))
    }
    ...
    // The chip is its own flex item in the composer dock, seated after the
    // built-in context pill: the dock flattens its contents wrappers, so the
    // item's flex `order` wins over DOM order. Its skin repeats the built-in
    // pill tokens and the shared stat-dialog panel tokens; both are --dsw-*
    // semantic aliases, never literal colors.
    const CSS = `
.dou-anchor {
  display: inline-flex;
  flex: none;
...
.dou-panel {
  ...
  border: 0;
  border-radius: 12px;
  /* Opaque composite: the built-in dialogs frost the translucent menu token
     over the page; this card keeps that surface color but never lets content
     behind it show through. */
  background:
    linear-gradient(var(--dsw-specific-menu), var(--dsw-specific-menu)),
...
`
     ...
     * @returns the chip as its own dock item, or null while there is nothing to show.
     */
    function QuotaChip(props) {
      ...
      if (state !== null && rows.length === 0) {
        return h('span', { ref: rootRef, className: 'dou-anchor' },
